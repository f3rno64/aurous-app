...
public class YouTubeEngine extends SearchEngine {
	...
	public String buildSearchCSV(final String json) {
		...
		for (int i = 0; i < jsonArray.length(); i++) {
			...
			final JSONObject jsonResults = jsonArray.getJSONObject(i);

			final String channel = jsonResults.getString("artist");
			...
			final String duration = jsonResults.getString("duration");
			...
			final String line = String.format("%s,%s,%s,%s,%s", title, artist,
					MediaUtils.calculateTime((int) Duration.parse(duration)
							.getSeconds()), url, id);
	...
	public String getArtist(final String title, final String channel) {
